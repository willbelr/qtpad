...
class Child(QtWidgets.QWidget):
    def __init__(self, parent, path, isNew=False, image=None, text=None):
        ...
        # Custom attributes according to note type (image or text)
        if image:
            self.isImage = True
    ...
    def renameEvent(self):
        ...
        msg.setTextValue(self.name)
        msg.setFixedSize(250, 100)
        accept = msg.exec_()
        result = msg.textValue()
        if accept and result:
            result = "".join(x for x in result if x.isalnum() or x is " ")
            if result and not result == self.name and result not in self.parent.children:
                self.rename(result)
